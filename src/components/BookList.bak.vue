  ...
  <div class="data-display">
    <a-table :columns="columns" :dataSource="data" :rowKey="record => record.id" :pagination="pagination">
      <template slot="action" slot-scope="text, record">
        ...
        -->
        <a href="javascript:;" @click="() => queryBookById(record.id, text)">详情</a>
  ...
  export default {
    ...
    methods: {
      ...
      queryBookById(bookId) {
        ...
        window.console.log(this.$route.path, this.bookQueryFrom);
        this.$store.commit('saveQueryParams', {path: this.$route.path, param: this.bookQueryFrom});
    ...
    beforeMount: function() {
      ...
      // 如果有缓存的查询条件，则初始化进去
      let queryForm = this.$store.state.queryParams.get(this.$route.path);
      window.console.log('cached queryform', queryForm);
      if (queryForm) {
        // this.bookQueryFrom = queryForm
      }
    }
  };
</script>
